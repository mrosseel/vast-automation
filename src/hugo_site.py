...
def block(star: StarDescription, resultdir: str, images_prefix: str):
    try:
        ...
        nl = "\n"
        name = (
            f"{nl.join(parsed_toml['our_name'])}"
        ...
        var_type = (
            f"{parsed_toml['var_type']}" if "var_type" in parsed_toml else UNKNOWN
        )
        vsx_var_flag = (
            f" ({parsed_toml['vsx_var_flag']})" if "vsx_var_flag" in parsed_toml else ""
        )
        tomlseparation = (
            parsed_toml["separation"] if "separation" in parsed_toml else None
        )
        ucacseparation = (
            star.coords.separation(star.get_metadata("UCAC4").coords).degree
            if star.has_metadata("UCAC4")
            else None
        )
        realseparation = (
            ucacseparation
            if ucacseparation is not None
            else tomlseparation
            if tomlseparation is not None
            else None
        )
        separation = (
            f"<li>separation: +/- {realseparation * 3600:.0f} arcsec</li>"
            if realseparation is not None
            else ""
        )
        var_type_link = (
            f"<a href='https://www.aavso.org/vsx/index.php?view=help.vartype&nolayout=1&abbrev="
            f"{var_type}'>{var_type}</a>"
            if var_type != UNKNOWN
            else var_type
        )
