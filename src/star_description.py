...
class StarDescription:
    def __init__(
        ...
        vmag: float = None,
        vmag_err: float = None,
        metadata: Dict[str, StarMetaData] = None,
        label: str = None,
        xpos: float = None,
        ypos: float = None,
        path: str = None,
    ...
    def set_metadata(self, val: StarMetaData, strict=True):
        if strict and val.key in self._metadata:
            error = (
                f"Tried to add metadata with key {val.key} but this is already present."
            )
    ...
    def __str__(self):
        return (
            f"local_id: {self.local_id}, aavso_id: {self.aavso_id}, coords: {self.coords}, xy: {self.xpos}, "
