...
def write_matching_config(filename, max_stars, vertices, clip_thresh, sp_fields, sp_maxoffset):
    a={ 'max_stars':max_stars,'vertices':vertices,'clip_thresh':clip_thresh,'sp_fields':sp_fields,'sp_maxoffset':sp_maxoffset}
    write_config(filename, a)
...
def perform_matching(configfilelist, all_photometry_files, reference_frame, offset=1):
    ...
    for id, entry in enumerate(configfilelist):
        ...
        for photfile in all_photometry_files:
            do_muniwin.write_match(photfile, reference_frame, config_file=entry, to_match_is_full_path=True, outputdir=outputdir)
...
def analyse(resultdirs=None, apertureidx=None):
    ...
    for resultdir in resultdirs:
        ...
        print("Reading dir:", resultdir)
        jdphot, fwhmphot, nrstarsphot, star_result = read_photometry.read_photometry(star_list_1=init.star_list, apertureidx=apertureidx, matched_files=glob.glob(resultdir+'/*.pht'), fake_references=True)
        print(f"Results: nr stars photometry:{len(nrstarsphot)}, sum of {np.sum(nrstarsphot)}, length of star list:{len(init.star_list)}")
        starsdetectedpct = np.sum(nrstarsphot) / (len(init.star_list) * len(nrstarsphot)) * 100
        print(star_result.flatten())
...
def analyse_match(resultdirs=None, apertureidx=None):
    ...
    for resultdir in resultdirs:
        # reading photometry files
        print("Reading dir:", resultdir)
        jdphot, fwhmphot, nrstarsphot, star_result = read_photometry.read_photometry(star_list_1=init.star_list, apertureidx=apertureidx, matched_files=glob.glob(resultdir+'/*.pht'), fake_references=False)
        print(f"Results: nr stars photometry:{len(nrstarsphot)}, sum of {np.sum(nrstarsphot)}, length of star list:{len(init.star_list)}")
        starsdetectedpct = np.sum(nrstarsphot) / (len(init.star_list) * len(nrstarsphot)) * 100
        print(star_result.flatten())
        print("number of finite star results:", np.sum(np.isfinite(star_result.flatten().shape)))
