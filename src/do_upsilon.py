...
def start_upsilon(star_list, star_limit):
    logging.info(
        f"Starting upsilon with nr stars: {len(star_list)} and star limit: {star_limit}"
    )
    result_list = []
...
def predict_star(star, limit=-1):
    ...
    try:
        ...
        date, mag, err = upsilon.utils.sigma_clipping(
            date, mag, err, threshold=3, iteration=1
        )
...
def run(star_list, outputdir: str):
    ...
    sorted_result_list = sorted(
        unsorted_result_list, key=lambda result: result[2], reverse=True
    )
