...
def convert(args):
    ...
    data = pd.read_fwf(path, colspecs=col_specification, names=(
    ...
    for index, row in tqdm.tqdm(data.iterrows(), total=len(data), desc="Converting"):
        extradata.append(
            {'id': index, 'OID': row['OID'], 'Name': row['Name'], 'Type': row['Type'], 'l_Period': row['l_Period'],
             'Period': row['Period'], 'u_Period': row['u_Period'], 'max': row['max'], 'min': row['min'],
             'epoch': row['Epoch']})
