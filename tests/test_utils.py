...
class TestUtils(unittest.TestCase):
    ...
    def test_metadata_sorter(self):
        ...
        self.assertEqual(
            15, utils.metadata_sorter.get_string_number_part_or_default("oenutheo15")
        )
        self.assertEqual(
            1, utils.metadata_sorter.get_string_number_part_or_default("RMH-HMB-1")
        )
        self.assertEqual(
            7, utils.metadata_sorter.get_string_number_part_or_default("RMH-NEW-7")
        )
        self.assertEqual(
            sys.maxsize,
            utils.metadata_sorter.get_string_number_part_or_default("RMH-NEW"),
        )

        stars = []
        for idx in range(1, 101):
            curr_star = self.stardesc(idx, random.random() * 360, random.random() * 90)
            curr_star.metadata = star_metadata.CatalogData(
                key="RMH-HMB",
                catalog_id=f"RMH-HMB-{idx}",
                name=f"RMH-HMB-{idx}",
                coords=curr_star.coords,
                separation=0,
            )
            stars.append(curr_star)
        random.shuffle(stars)
        result = utils.metadata_sorter(
            stars, metadata_id="RMH-HMB", name_variable="name"
        )
    ...
    def test_metadata_filter(self):
        ...
        result = utils.metadata_filter(
            self.star_descriptions[0], catalog_name="CANDIDATES", exclude=["VSX"]
        )
        self.assertFalse(result)
        result = utils.metadata_filter(
            self.star_descriptions[1], catalog_name="CANDIDATES", exclude=["VSX"]
        )
        self.assertTrue(result)
        result = utils.metadata_filter(
            self.star_descriptions[1], catalog_name="CANDIDATES", exclude=[]
        )
        self.assertTrue(result)
        result = utils.metadata_filter(
            self.star_descriptions[2], catalog_name="VSX", exclude=[]
        )
        self.assertTrue(result)
        try:
            result = utils.metadata_filter(
                self.star_descriptions[2], catalog_name="VSX", exclude="VSX"
            )
    ...
    def set_catalog(star, catalog_name: str):
        star.metadata = CatalogData(
            key=catalog_name,
            catalog_id=f"{catalog_name}-{star.local_id}",
            name=f"{catalog_name}-{star.local_id}",
            separation=None,
            coords=star.coords,
            extradata=None,
        )
