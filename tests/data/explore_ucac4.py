...
def process_file(uc, zone_nr, bucket, process_func):
    ...
    for run_nr in run_nrs:
        ...
        result = uc.unpack_zone_fileformat(
            filecontent[
                uc.zone_star_length * run_nr : uc.zone_star_length * (run_nr + 1)
            ]
        )
...
def get_ra_decs(uc, zone_nr, bucket, run_nrs, stars):
    ...
    logging.debug(
        f"Zone {zone_nr}: between ra<{min(ras)}, {max(ras)}> and  dec<f{min(decs)}, {max(decs)}>, "
